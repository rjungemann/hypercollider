String[char] : RawArray {
	...
	*doUnixCmdAction { arg res, pid;
	...
	unixCmdGetStdOut { arg maxLineLength=1024;
		...
		if (thisProcess.platform.name == \wasm) {
			^this.prUnixCmdGetStdOutWasm(maxLineLength);
		};
	...
	prUnixCmdGetStdOutWasm { arg maxLineLength = 1024;
		_String_UnixCmdGetStdOutWasm
		^this.primitiveFailed
	}
	...
	*readNew { arg file;
	...
	basename {
		...
		if (lastSep < 0) { ^this };
		if (lastSep >= (this.size - 1)) { ^"" };
		^this.copyToEnd(lastSep + 1)
	}
	dirname {
		var lastSep;
		lastSep = -1;
		...
		if (lastSep < 0) { ^"." };
		if (lastSep == 0) { ^this[0].asString };
		^this.copyRange(0, lastSep - 1)
	}
	splitext {
