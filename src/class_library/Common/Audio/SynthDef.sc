	...
	*writeOnce { arg name, func, rates, prependArgs, variants, dir, metadata, mdPlugin;
	...
	buildControls {
		...
		if (irControlNames.size > 0) {
			values = Array.new(irControlNames.size);
		...
		if (trControlNames.size > 0) {
			values = Array.new(trControlNames.size);
		...
		if (arControlNames.size > 0) {
			values = Array.new(arControlNames.size);
		...
		if (krControlNames.size > 0) {
			values = Array.new(krControlNames.size);
			lags = Array.new(krControlNames.size);
	...
	addConstant { arg value;
		if (constants.includesKey(value).not) {
