...
void FilterDelay_Dtor(FilterDelayUnit* unit) { RTFree(unit->mWorld, unit->m_dlybuf); }
...
void NLFiltN_next(NLFiltN* unit, int inNumSamples) {
    ...
    if (next_a != a || next_b != b || next_d != d || next_c != c || next_l != l) {
        ...
        LOOP(inNumSamples, a += aslope; b += bslope; d += dslope; c += cslope; l += lslope;
             float delay1 = dlybuf[(iwrphase - 1) & mask]; float delay2 = dlybuf[(iwrphase - 2) & mask];
             float delayL = dlybuf[(iwrphase - (long)l) & mask];

             float output = ZXP(in) + a * delay1 + b * delay2 + d * delayL * delayL - c;
             output = output - (output * output * output * 0.166666667); output = Mirroring(-1.0, 1.0, output);
             ZXP(out) = dlybuf[iwrphase & mask] = output; iwrphase++;);
    } else {
        LOOP(inNumSamples, float delay1 = dlybuf[(iwrphase - 1) & mask]; float delay2 = dlybuf[(iwrphase - 2) & mask];
             float delayL = dlybuf[(iwrphase - (long)l) & mask];
...
void NLFiltN_next_z(NLFiltN* unit, int inNumSamples) {
    ...
    if (next_a != a || next_b != b || next_d != d || next_c != c || next_l != l) {
        ...
        float cslope = CALCSLOPE(next_c, c);
        LOOP(
            inNumSamples, a += aslope; b += bslope; d += dslope; c += cslope; l += lslope;
            long irphase = iwrphase - (long)l; float delay1 = dlybuf[(iwrphase - 1) & mask];
            float delay2 = dlybuf[(iwrphase - 2) & mask]; float delayL = dlybuf[irphase & mask];

            float input = ZXP(in); float output = input + a * delay1 + b * delay2 + d * delayL * delayL - c;
            output = output - (output * output * output * 0.166666667); output = Mirroring(-1.0, 1.0, output);
            if (irphase < 0) {
                dlybuf[iwrphase & mask] = input;
                ZXP(out) = 0.f;
            } else { ZXP(out) = dlybuf[iwrphase & mask] = output; } iwrphase++;);
    } else {
        LOOP(
            inNumSamples, long irphase = iwrphase - (long)l; float delay1 = dlybuf[(iwrphase - 1) & mask];
            float delay2 = dlybuf[(iwrphase - 2) & mask]; float delayL = dlybuf[irphase & mask];
...
void NLFiltL_next(NLFiltL* unit, int inNumSamples) {
    ...
    if (next_a != a || next_b != b || next_d != d || next_c != c || next_l != l) {
        ...
        float dslope = CALCSLOPE(next_d, d);
        float cslope = CALCSLOPE(next_c, c);
        LOOP(inNumSamples, l += lslope; a += aslope; b += bslope; d += dslope; c += cslope;
             float delay1 = dlybuf[(iwrphase - 1) & mask]; float delay2 = dlybuf[(iwrphase - 2) & mask];
             long l1 = (long)l; float frac = l - l1; float delayL1 = dlybuf[(iwrphase - l1) & mask];
             float delayL2 = dlybuf[(iwrphase - l1 - 1) & mask]; float delayL = lininterp(frac, delayL1, delayL2);
...
void NLFiltL_next_z(NLFiltL* unit, int inNumSamples) {
    ...
    if (next_a != a || next_b != b || next_d != d || next_c != c || next_l != l) {
        ...
        float cslope = CALCSLOPE(next_c, c);
        LOOP(
            inNumSamples, l += lslope; a += aslope; b += bslope; d += dslope; c += cslope;
            float delay1 = dlybuf[(iwrphase - 1) & mask]; float delay2 = dlybuf[(iwrphase - 2) & mask];
            long l1 = (long)l; float frac = l - l1; long irphase = iwrphase - l1;
            float delayL1 = dlybuf[irphase & mask]; float delayL2 = dlybuf[(irphase - 1) & mask];
            float delayL = lininterp(frac, delayL1, delayL2); float input = ZXP(in);
        ...
        LOOP(
            inNumSamples, float delay1 = dlybuf[(iwrphase - 1) & mask]; float delay2 = dlybuf[(iwrphase - 2) & mask];
            long l1 = (long)l; float frac = l - l1; long irphase = iwrphase - l1;
            float delayL1 = dlybuf[irphase & mask]; float delayL2 = dlybuf[(irphase - 1) & mask];
            float delayL = lininterp(frac, delayL1, delayL2); float input = ZXP(in);
            float output = input + a * delay1 + b * delay2 + d * delayL * delayL - c;
...
void NLFiltC_next(NLFiltC* unit, int inNumSamples) {
    ...
    if (next_a != a || next_b != b || next_d != d || next_c != c || next_l != l) {
        ...
        LOOP(inNumSamples, l += lslope; a += aslope; b += bslope; d += dslope; c += cslope;
             float delay1 = dlybuf[(iwrphase - 1) & mask]; float delay2 = dlybuf[(iwrphase - 2) & mask];
             long l1 = (long)l; float frac = l - l1; long irdphase1 = iwrphase - l1; long irdphase2 = irdphase1 - 1;
             long irdphase3 = irdphase1 - 2; long irdphase0 = irdphase1 + 1; float d0 = dlybuf[irdphase0 & mask];
             float d1 = dlybuf[irdphase1 & mask]; float d2 = dlybuf[irdphase2 & mask];
             float d3 = dlybuf[irdphase3 & mask]; float delayL = cubicinterp(frac, d0, d1, d2, d3);
...
void Streson_Ctor(Streson* unit) {
    ...
    if (INRATE(1) == calc_FullRate)
        SETCALC(Streson_next_a_z);
    else
        SETCALC(Streson_next_k_z);
    ZOUT0(0) = 0.f;
}
...
void Streson_next_k(Streson* unit, int inNumSamples) {
    ...
    if (next_delaytime != delaytime) {
        ...
        LOOP(inNumSamples, long irdphase = iwrphase - (long)dsamp; float input = ZXP(in) + dlybuf[irdphase & mask];
             float s = lp * 0.5 + input * 0.5; lp = input; float sample = ap + s * a; ZXP(out) = sample;
             ap = s - (sample * a); dlybuf[iwrphase & mask] = sample * ZIN0(2); ++iwrphase;);
    } else {
        LOOP(inNumSamples, long irdphase = iwrphase - (long)dsamp; float input = ZXP(in) + dlybuf[irdphase & mask];
             float s = lp * 0.5 + input * 0.5; lp = input; float sample = ap + s * a; ZXP(out) = sample;
             ap = s - (sample * a); dlybuf[iwrphase & mask] = sample * ZIN0(2); ++iwrphase;);
...
void Streson_next_k_z(Streson* unit, int inNumSamples) {
    ...
    if (next_delaytime != delaytime) {
        ...
        float a = (1 - fracdelay) / (1 + fracdelay);
        LOOP(
            inNumSamples, long irdphase = iwrphase - (long)dsamp; float aud = ZXP(in);
            float input = aud + dlybuf[irdphase & mask]; float s = lp * 0.5 + input * 0.5; lp = input;
            float sample = ap + s * a; ap = s - (sample * a); if (irdphase < 0) {
...
void Streson_next_a(Streson* unit, int inNumSamples) {
    ...
    LOOP(inNumSamples, float tdelay = ZXP(delaytime) * SAMPLERATE; float dsamp = (long)(tdelay - 0.5);
         float fracdelay = tdelay - (dsamp + 0.5); float a = (1 - fracdelay) / (1 + fracdelay);
         long irdphase = iwrphase - (long)dsamp; float input = ZXP(in) + dlybuf[irdphase & mask];
         float s = lp * 0.5 + input * 0.5; lp = input; float sample = ap + s * a; ZXP(out) = sample;
         ap = s - (sample * a); dlybuf[iwrphase & mask] = sample * ZIN0(2); ++iwrphase;);
...
void Streson_next_a_z(Streson* unit, int inNumSamples) {
    ...
    LOOP(
        inNumSamples, float tdelay = ZXP(delaytime) * SAMPLERATE; float dsamp = (long)(tdelay - 0.5);
        float fracdelay = tdelay - (dsamp + 0.5); float a = (1 - fracdelay) / (1 + fracdelay);
        long irdphase = iwrphase - (long)dsamp; float aud = ZXP(in); float input = aud + dlybuf[irdphase & mask];
        float s = lp * 0.5 + input * 0.5; lp = input; float sample = ap + s * a; ap = s - (sample * a);
        if (irdphase < 0) {
            dlybuf[iwrphase & mask] = aud;
            ZXP(out) = 0.f;
        } else {
...
void MultiFilt_next_kk(MultiFilt* unit, int inNumSamples) {
    ...
    if (nextcf != unit->m_cf) {
        ...
        if (nextrq != rq) {
            if (nextrq * unit->m_halfsamp < nextcf)
                nextrq = nextcf / unit->m_halfsamp;
            float rqslope = CALCSLOPE(nextrq, rq);
            LOOP(inNumSamples, float sig = ZXP(in); hp = sig - rq * bp - lp; lp = bp * f + lp; bp = hp * f + bp;

                 hp = sig - rq * bp - lp; lp = bp * f + lp; bp = hp * f + bp;
                 ...
                 ZXP(out[0]) = lp; ZXP(out[1]) = hp; ZXP(out[2]) = bp;);
        } else {
            if (rq * unit->m_halfsamp < nextcf)
                rq = nextcf / unit->m_halfsamp;
            LOOP(inNumSamples, float sig = ZXP(in); hp = sig - rq * bp - lp; lp = bp * f + lp; bp = hp * f + bp;
...
void MultiFilt_next_ak(MultiFilt* unit, int inNumSamples) {
    ...
    if (nextrq != rq) {
        float rqslope = CALCSLOPE(nextrq, rq);
        LOOP(
            inNumSamples, cf = ZXP(nextcf); if (cf != lastcf) {
            ...
            float sig = ZXP(in); hp = sig - rq * bp - lp; lp = bp * f + lp; bp = hp * f + bp;
...
void MultiFilt_next_ka(MultiFilt* unit, int inNumSamples) {
    ...
    if (nextcf != unit->m_cf) {
        ...
        LOOP(inNumSamples, rq = ZXP(nextrq); if (rq * unit->m_halfsamp < nextcf) rq = nextcf / unit->m_halfsamp;
             float sig = ZXP(in); hp = sig - rq * bp - lp; lp = bp * f + lp; bp = hp * f + bp;
...
void MultiFilt_next_aa(MultiFilt* unit, int inNumSamples) {
    ...
    LOOP(
        inNumSamples, cf = ZXP(nextcf); rq = ZXP(nextrq); if (cf != lastcf) {
            f = 2.f * sin(pi * cf * SAMPLEDUR * 0.25);
            lastcf = cf;
        }
...
void RLPFD_next_kk(RLPFD* unit, int inNumSamples) {
    ...
    for (int i = 0; i < inNumSamples; i++) {
        ...
        if (dist > 0.f)
            y3 = y3 * (1.f + (dist * (1.5 + 2.f * kres * (1.f - fcn))));
        y3 = y3 / sqrt(1 + y3 * y3);
...
void NestedAllpassN_next_z(NestedAllpassN* unit, int inNumSamples) {
    ...
    if (next_dt1 == dt1 && next_dt2 == dt2) {
        LOOP(
            inNumSamples, phase++; dsamp1++; dsamp2++; samp = ZXP(in); if (dsamp1 < 0 || dsamp2 < 0) {
        ...
        LOOP(
            inNumSamples, phase++; dt1 += dslope1; dt2 += dslope2;
            dsamp1 = phase - (long)(dt1 * unit->mRate->mSampleRate);
            dsamp2 = phase - (long)(dt2 * unit->mRate->mSampleRate); samp = ZXP(in);
            buf1[phase & mask1] = samp + gain1 * output; buf2[phase & mask2] = del1 + gain2 * sum;
...
void NestedAllpassN_next(NestedAllpassN* unit, int inNumSamples) {
    ...
    if (next_dt1 == dt1 && next_dt2 == dt2) {
        LOOP(inNumSamples, phase++; dsamp1++; dsamp2++; samp = ZXP(in); del1 = buf1[dsamp1 & mask1];
             sum = buf2[dsamp2 & mask2] - gain2 * del1; output = sum - gain1 * samp;
             buf1[phase & mask1] = samp + gain1 * output; buf2[phase & mask2] = del1 + gain2 * sum; ZXP(out) = output;);
    } else {
        float dslope1 = CALCSLOPE(next_dt1, dt1);
        float dslope2 = CALCSLOPE(next_dt2, dt2);
        LOOP(inNumSamples, phase++; dt1 += dslope1; dt2 += dslope2;
             dsamp1 = phase - (long)(dt1 * unit->mRate->mSampleRate);
             dsamp2 = phase - (long)(dt2 * unit->mRate->mSampleRate); samp = ZXP(in); del1 = buf1[dsamp1 & mask1];
             sum = buf2[dsamp2 & mask2] - gain2 * del1; output = sum - gain1 * samp;
             buf1[phase & mask1] = samp + gain1 * output; buf2[phase & mask2] = del1 + gain2 * sum; ZXP(out) = output;);
    }
...
void NestedAllpassL_next(NestedAllpassL* unit, int inNumSamples) {
    ...
    if (next_dt1 == dt1 && next_dt2 == dt2) {
        int idsamp1;
        int idsamp2;
        LOOP(inNumSamples, phase++; dsamp1++; dsamp2++; idsamp1 = (int)dsamp1; idsamp2 = (int)dsamp2;
             frac1 = dsamp1 - idsamp1; frac2 = dsamp2 - idsamp2; samp = ZXP(in); del11 = buf1[idsamp1 & mask1];
             del21 = buf2[idsamp2 & mask2]; float del12 = buf1[(idsamp1 - 1) & mask1];
             float del22 = buf2[(idsamp2 - 1) & mask2]; del1 = lininterp(frac1, del11, del12);
             sum = lininterp(frac2, del21, del22) - gain2 * del1; output = sum - gain1 * samp;
             buf1[phase & mask1] = samp + gain1 * output; buf2[phase & mask2] = del1 + gain2 * sum; ZXP(out) = output;);
    } else {
        float dslope1 = CALCSLOPE(next_dt1, dt1);
        float dslope2 = CALCSLOPE(next_dt2, dt2);
        LOOP(inNumSamples, phase++; dt1 += dslope1; dt2 += dslope2; dsamp1 = phase - dt1 * unit->mRate->mSampleRate;
             dsamp2 = phase - dt2 * unit->mRate->mSampleRate; int idsamp1 = (int)dsamp1; int idsamp2 = (int)dsamp2;
             frac1 = dsamp1 - idsamp1; frac2 = dsamp2 - idsamp2; samp = ZXP(in);
             ...
             output = sum - gain1 * samp; buf1[phase & mask1] = samp + gain1 * output;
             buf2[phase & mask2] = del1 + gain2 * sum; ZXP(out) = output;);
    }
...
void NestedAllpassL_next_z(NestedAllpassL* unit, int inNumSamples) {
    ...
    if (next_dt1 == dt1 && next_dt2 == dt2) {
        ...
        int idsamp2;
        LOOP(
            inNumSamples, phase++; dsamp1++; dsamp2++; idsamp1 = (int)dsamp1; idsamp2 = (int)dsamp2;
            frac1 = dsamp1 - idsamp1; frac2 = dsamp2 - idsamp2; samp = ZXP(in); del11 = buf1[idsamp1 & mask1];
            del21 = buf2[idsamp2 & mask2]; float del12 = buf1[(idsamp1 - 1) & mask1];
            float del22 = buf2[(idsamp2 - 1) & mask2]; del1 = lininterp(frac1, del11, del12);
            sum = lininterp(frac2, del21, del22) - gain2 * del1; output = sum - gain1 * samp;
        ...
        float dslope2 = CALCSLOPE(next_dt2, dt2);
        LOOP(
            inNumSamples, phase++; dt1 += dslope1; dt2 += dslope2; dsamp1 = phase - dt1 * unit->mRate->mSampleRate;
            dsamp2 = phase - dt2 * unit->mRate->mSampleRate; int idsamp1 = (int)dsamp1; int idsamp2 = (int)dsamp2;
            frac1 = dsamp1 - idsamp1; frac2 = dsamp2 - idsamp2; samp = ZXP(in); del11 = buf1[idsamp1 & mask1];
            del21 = buf2[idsamp2 & mask2]; del1 = lininterp(frac1, del11, buf1[(idsamp1 - 1) & mask1]);
            sum = lininterp(frac2, del21, buf2[(idsamp2 - 1) & mask2]) - gain2 * del1; output = sum - gain1 * samp;
            if ((idsamp1 - 1) < 0 || (idsamp2 - 1) < 0) {
                buf1[phase & mask1] = samp;
                buf2[phase & mask2] = samp;
                ZXP(out) = 0.f;
...
void NestedAllpassC_next(NestedAllpassC* unit, int inNumSamples) {
    ...
    if (next_dt1 == dt1 && next_dt2 == dt2) {
        ...
        LOOP(inNumSamples, phase++; dsamp1++; dsamp2++; idsamp1 = (int)dsamp1; idsamp2 = (int)dsamp2;
             frac1 = dsamp1 - idsamp1; frac2 = dsamp2 - idsamp2; samp = ZXP(in);
             float del10 = buf1[(idsamp1 + 1) & mask1]; float del11 = buf1[idsamp1 & mask1];
             float del12 = buf1[(idsamp1 - 1) & mask1]; float del13 = buf1[(idsamp1 - 2) & mask1];
             del1 = cubicinterp(frac1, del10, del11, del12, del13); float del20 = buf2[(idsamp2 + 1) & mask2];
             float del21 = buf2[idsamp2 & mask2]; float del22 = buf2[(idsamp2 - 1) & mask2];
             float del23 = buf2[(idsamp2 - 2) & mask2]; del2 = cubicinterp(frac1, del20, del21, del22, del23);
             sum = del2 - gain2 * del1; output = sum - gain1 * samp; buf1[phase & mask1] = samp + gain1 * output;
             buf2[phase & mask2] = del1 + gain2 * sum; ZXP(out) = output;);
    } else {
        float dslope1 = CALCSLOPE(next_dt1, dt1);
        float dslope2 = CALCSLOPE(next_dt2, dt2);
        LOOP(inNumSamples, phase++; dt1 += dslope1; dt2 += dslope2; dsamp1 = phase - dt1 * unit->mRate->mSampleRate;
             dsamp2 = phase - dt2 * unit->mRate->mSampleRate; int idsamp1 = (int)dsamp1; int idsamp2 = (int)dsamp2;
             frac1 = dsamp1 - idsamp1; frac2 = dsamp2 - idsamp2; samp = ZXP(in); float del11 = buf1[idsamp1 & mask1];
             float del12 = buf1[(idsamp1 - 1) & mask1]; float del13 = buf1[(idsamp1 - 2) & mask1];
             float del10 = buf1[(idsamp1 + 1) & mask1]; del1 = cubicinterp(frac1, del10, del11, del12, del13);
             float del21 = buf2[idsamp2 & mask2]; float del22 = buf2[(idsamp2 - 1) & mask2];
             float del23 = buf2[(idsamp2 - 2) & mask2]; float del20 = buf2[(idsamp2 + 1) & mask2];
             del2 = cubicinterp(frac1, del20, del21, del22, del23); sum = del2 - gain2 * del1;
             output = sum - gain1 * samp; buf1[phase & mask1] = samp + gain1 * output;
             buf2[phase & mask2] = del1 + gain2 * sum; ZXP(out) = output;);
    }
...
void NestedAllpassC_next_z(NestedAllpassC* unit, int inNumSamples) {
    ...
    if (next_dt1 == dt1 && next_dt2 == dt2) {
        ...
        LOOP(
            ...
            float del21 = buf2[idsamp2 & mask2]; float del22 = buf2[(idsamp2 - 1) & mask2];
            float del23 = buf2[(idsamp2 - 2) & mask2]; del2 = cubicinterp(frac1, del20, del21, del22, del23);
            sum = del2 - gain2 * del1; output = sum - gain1 * samp; if (idsamp1 < 0 || idsamp2 < 0) {
                buf1[phase & mask1] = samp;
                buf2[phase & mask2] = samp;
                ZXP(out) = 0.f;
            } else {
        ...
        LOOP(
            ...
            float del23 = buf2[(idsamp2 - 2) & mask2]; float del20 = buf2[(idsamp2 + 1) & mask2];
            del2 = cubicinterp(frac1, del20, del21, del22, del23); sum = del2 - gain2 * del1;
            output = sum - gain1 * samp; if (idsamp1 < 0 || idsamp2 < 0) {
                buf1[phase & mask1] = samp;
                buf2[phase & mask2] = samp;
                ZXP(out) = 0.f;
            } else {
...
void DoubleNestedAllpassN_next(DoubleNestedAllpassN* unit, int inNumSamples) {
    ...
    if (next_dt1 == dt1 && next_dt2 == dt2 && next_dt3 == dt3) {
        LOOP(inNumSamples, phase++; dsamp1++; dsamp2++; dsamp3++; samp = ZXP(in); del1 = buf1[dsamp1 & mask1];
             sum1 = buf2[dsamp2 & mask2] - gain2 * del1; sum2 = buf3[dsamp3 & mask3] - gain3 * sum1;
             output = sum2 - gain1 * samp; buf1[phase & mask1] = samp + gain1 * output;
             buf2[phase & mask2] = del1 + gain2 * sum1; buf3[phase & mask3] = sum1 + gain3 * sum2; ZXP(out) = output;);
    } else {
        float dslope1 = CALCSLOPE(next_dt1, dt1);
        float dslope2 = CALCSLOPE(next_dt2, dt2);
        float dslope3 = CALCSLOPE(next_dt3, dt3);
        LOOP(inNumSamples, phase++; dt1 += dslope1; dt2 += dslope2; dt3 += dslope3;
             dsamp1 = phase - (long)(dt1 * unit->mRate->mSampleRate);
             dsamp2 = phase - (long)(dt2 * unit->mRate->mSampleRate);
             dsamp3 = phase - (long)(dt3 * unit->mRate->mSampleRate); samp = ZXP(in); del1 = buf1[dsamp1 & mask1];
             sum1 = buf2[dsamp2 & mask2] - gain2 * del1; sum2 = buf3[dsamp3 & mask3] - gain3 * sum1;
             output = sum2 - gain1 * samp; buf1[phase & mask1] = samp + gain1 * output;
             buf2[phase & mask2] = del1 + gain2 * sum1; buf3[phase & mask3] = sum1 + gain3 * sum2; ZXP(out) = output;);
    }
...
void DoubleNestedAllpassN_next_z(DoubleNestedAllpassN* unit, int inNumSamples) {
    ...
    if (next_dt1 == dt1 && next_dt2 == dt2 && next_dt3 == dt3) {
        LOOP(
            inNumSamples, phase++; dsamp1++; dsamp2++; dsamp3++; samp = ZXP(in); del1 = buf1[dsamp1 & mask1];
            sum1 = buf2[dsamp2 & mask2] - gain2 * del1; sum2 = buf3[dsamp3 & mask3] - gain3 * sum1;
            output = sum2 - gain1 * samp; if (dsamp1 < 0 || dsamp2 < 0 || dsamp3 < 0) {
                buf1[phase & mask1] = samp;
                buf2[phase & mask2] = samp;
        ...
        float dslope3 = CALCSLOPE(next_dt3, dt3);
        LOOP(
            inNumSamples, phase++; dt1 += dslope1; dt2 += dslope2; dt3 += dslope3;
            dsamp1 = phase - (long)(dt1 * unit->mRate->mSampleRate);
            dsamp2 = phase - (long)(dt2 * unit->mRate->mSampleRate);
            dsamp3 = phase - (long)(dt3 * unit->mRate->mSampleRate); samp = ZXP(in); del1 = buf1[dsamp1 & mask1];
            sum1 = buf2[dsamp2 & mask2] - gain2 * del1; sum2 = buf3[dsamp3 & mask3] - gain3 * sum1;
...
void DoubleNestedAllpassL_next(DoubleNestedAllpassL* unit, int inNumSamples) {
    ...
    if (next_dt1 == dt1 && next_dt2 == dt2 && next_dt3 == dt3) {
        ...
        int idsamp2;
        int idsamp3;
        LOOP(inNumSamples, phase++; dsamp1++; dsamp2++; dsamp3++; idsamp1 = (int)dsamp1; float frac1 = dsamp1 - idsamp1;
             idsamp2 = (int)dsamp2; float frac2 = dsamp2 - idsamp2; idsamp3 = (int)dsamp3;
             float frac3 = dsamp3 - idsamp3; samp = ZXP(in); float del10 = buf1[idsamp1 & mask1];
             float del20 = buf2[idsamp2 & mask2]; float del30 = buf3[idsamp3 & mask3];
             float del11 = buf1[(idsamp1 - 1) & mask1]; float del21 = buf2[(idsamp2 - 1) & mask2];
             float del31 = buf3[(idsamp3 - 1) & mask3]; del1 = lininterp(frac1, del10, del11);
             sum1 = lininterp(frac2, del20, del21) - gain2 * del1; sum2 = lininterp(frac3, del30, del31) - gain3 * sum1;
             output = sum2 - gain1 * samp; buf1[phase & mask1] = samp + gain1 * output;
             buf2[phase & mask2] = del1 + gain2 * sum1; buf3[phase & mask3] = sum1 + gain3 * sum2; ZXP(out) = output;);
    } else {
        float dslope1 = CALCSLOPE(next_dt1, dt1);
        ...
        LOOP(inNumSamples, phase++; dt1 += dslope1; dt2 += dslope2; dt3 += dslope3;
             dsamp1 = phase - (long)(dt1 * unit->mRate->mSampleRate);
             dsamp2 = phase - (long)(dt2 * unit->mRate->mSampleRate);
             dsamp3 = phase - (long)(dt3 * unit->mRate->mSampleRate); int idsamp1 = (int)dsamp1;
             float frac1 = dsamp1 - idsamp1; int idsamp2 = (int)dsamp2; float frac2 = dsamp2 - idsamp2;
             int idsamp3 = (int)dsamp3; float frac3 = dsamp3 - idsamp3; samp = ZXP(in);
             del1 = lininterp(frac1, buf1[idsamp1 & mask1], buf1[(idsamp1 - 1) & mask1]);
...
void DoubleNestedAllpassL_next_z(DoubleNestedAllpassL* unit, int inNumSamples) {
    ...
    if (next_dt1 == dt1 && next_dt2 == dt2 && next_dt3 == dt3) {
        ...
        int idsamp3;
        LOOP(
            inNumSamples, phase++; dsamp1++; dsamp2++; dsamp3++; idsamp1 = (int)dsamp1; float frac1 = dsamp1 - idsamp1;
            idsamp2 = (int)dsamp2; float frac2 = dsamp2 - idsamp2; idsamp3 = (int)dsamp3;
            float frac3 = dsamp3 - idsamp3; int idsamp11 = idsamp1 - 1; int idsamp21 = idsamp2 - 1;
            int idsamp31 = idsamp3 - 1; samp = ZXP(in); float del10 = buf1[idsamp1 & mask1];
            float del20 = buf2[idsamp2 & mask2]; float del30 = buf3[idsamp3 & mask3];
            float del11 = buf1[idsamp11 & mask1]; float del21 = buf2[idsamp21 & mask2];
            float del31 = buf3[idsamp31 & mask3]; del1 = lininterp(frac1, del10, del11);
            sum1 = lininterp(frac2, del20, del21) - gain2 * del1; sum2 = lininterp(frac3, del30, del31) - gain3 * sum1;
            output = sum2 - gain1 * samp; if (idsamp11 < 0 || idsamp21 < 0 || idsamp31 < 0) {
                buf1[phase & mask1] = samp;
                buf2[phase & mask2] = samp;
                buf3[phase & mask3] = samp;
                ZXP(out) = 0.f;
        ...
        LOOP(
            ...
            dsamp3 = phase - (long)(dt3 * unit->mRate->mSampleRate); int idsamp1 = (int)dsamp1;
            float frac1 = dsamp1 - idsamp1; int idsamp2 = (int)dsamp2; float frac2 = dsamp2 - idsamp2;
            int idsamp3 = (int)dsamp3; float frac3 = dsamp3 - idsamp3; int idsamp11 = idsamp1 - 1;
            int idsamp21 = idsamp2 - 1; int idsamp31 = idsamp3 - 1; samp = ZXP(in);
...
void DoubleNestedAllpassC_next(DoubleNestedAllpassC* unit, int inNumSamples) {
    ...
    if (next_dt1 == dt1 && next_dt2 == dt2 && next_dt3 == dt3) {
        ...
        LOOP(inNumSamples, phase++; dsamp1++; dsamp2++; dsamp3++; idsamp1 = (int)dsamp1; float frac1 = dsamp1 - idsamp1;
             idsamp2 = (int)dsamp2; float frac2 = dsamp2 - idsamp2; idsamp3 = (int)dsamp3;
             float frac3 = dsamp3 - idsamp3; samp = ZXP(in); float del10 = buf1[(idsamp1 + 1) & mask1];
             float del20 = buf2[(idsamp2 + 1) & mask2]; float del30 = buf3[(idsamp3 + 1) & mask3];
             float del11 = buf1[(idsamp1)&mask1]; float del21 = buf2[(idsamp2)&mask2];
             float del31 = buf3[(idsamp3)&mask3]; float del12 = buf1[(idsamp1 - 1) & mask1];
             float del22 = buf2[(idsamp2 - 1) & mask2]; float del32 = buf3[(idsamp3 - 1) & mask3];
             float del13 = buf1[(idsamp1 - 2) & mask1]; float del23 = buf2[(idsamp2 - 2) & mask2];
             float del33 = buf3[(idsamp3 - 2) & mask3]; del1 = cubicinterp(frac1, del10, del11, del12, del13);
             sum1 = cubicinterp(frac2, del20, del21, del22, del23) - gain2 * del1;
             sum2 = cubicinterp(frac3, del30, del31, del32, del33) - gain3 * sum1; output = sum2 - gain1 * samp;
             buf1[phase & mask1] = samp + gain1 * output; buf2[phase & mask2] = del1 + gain2 * sum1;
             buf3[phase & mask3] = sum1 + gain3 * sum2; ZXP(out) = output;);
    } else {
        float dslope1 = CALCSLOPE(next_dt1, dt1);
        float dslope2 = CALCSLOPE(next_dt2, dt2);
        float dslope3 = CALCSLOPE(next_dt3, dt3);
        LOOP(inNumSamples, phase++; dt1 += dslope1; dt2 += dslope2; dt3 += dslope3;
             ...
             float frac1 = dsamp1 - idsamp1; int idsamp2 = (int)dsamp2; float frac2 = dsamp2 - idsamp2;
             int idsamp3 = (int)dsamp3; float frac3 = dsamp3 - idsamp3; samp = ZXP(in);
             del1 = cubicinterp(frac1, buf1[(idsamp1 + 1) & mask1], buf1[idsamp1 & mask1], buf1[(idsamp1 - 1) & mask1],
                                buf1[(idsamp1 - 2) & mask1]);
             sum1 = cubicinterp(frac2, buf2[(idsamp2 + 1) & mask2], buf2[idsamp2 & mask2], buf2[(idsamp2 - 1) & mask2],
                                buf2[(idsamp2 - 2) & mask2])
                 - gain2 * del1;
             sum2 = cubicinterp(frac3, buf3[(idsamp3 + 1) & mask3], buf3[idsamp3 & mask3], buf3[(idsamp3 - 1) & mask3],
                                buf3[(idsamp3 - 2) & mask3])
                 - gain3 * sum1;
             output = sum2 - gain1 * samp; buf1[phase & mask1] = samp + gain1 * output;
             buf2[phase & mask2] = del1 + gain2 * sum1; buf3[phase & mask3] = sum1 + gain3 * sum2; ZXP(out) = output;);
    }
...
void DoubleNestedAllpassC_next_z(DoubleNestedAllpassC* unit, int inNumSamples) {
    ...
    if (next_dt1 == dt1 && next_dt2 == dt2 && next_dt3 == dt3) {
        ...
        LOOP(
            inNumSamples, phase++; dsamp1++; dsamp2++; dsamp3++; idsamp1 = (int)dsamp1; float frac1 = dsamp1 - idsamp1;
            idsamp2 = (int)dsamp2; float frac2 = dsamp2 - idsamp2; idsamp3 = (int)dsamp3;
            float frac3 = dsamp3 - idsamp3; int idsamp13 = idsamp1 - 2; int idsamp23 = idsamp2 - 2;
            int idsamp33 = idsamp3 - 2; samp = ZXP(in); float del10 = buf1[(idsamp1 + 1) & mask1];
            float del20 = buf2[(idsamp2 + 1) & mask2]; float del30 = buf3[(idsamp3 + 1) & mask3];
            float del11 = buf1[(idsamp1)&mask1]; float del21 = buf2[(idsamp2)&mask2];
            float del31 = buf3[(idsamp3)&mask3]; float del12 = buf1[(idsamp1 - 1) & mask1];
            float del22 = buf2[(idsamp2 - 1) & mask2]; float del32 = buf3[(idsamp3 - 1) & mask3];
            float del13 = buf1[idsamp13 & mask1]; float del23 = buf2[idsamp23 & mask2];
            float del33 = buf3[idsamp33 & mask3]; del1 = cubicinterp(frac1, del10, del11, del12, del13);
            sum1 = cubicinterp(frac2, del20, del21, del22, del23) - gain2 * del1;
            sum2 = cubicinterp(frac3, del30, del31, del32, del33) - gain3 * sum1; output = sum2 - gain1 * samp;
        ...
        LOOP(
            ...
            dsamp3 = phase - (long)(dt3 * unit->mRate->mSampleRate); int idsamp1 = (int)dsamp1;
            float frac1 = dsamp1 - idsamp1; int idsamp2 = (int)dsamp2; float frac2 = dsamp2 - idsamp2;
            int idsamp3 = (int)dsamp3; float frac3 = dsamp3 - idsamp3; int idsamp13 = idsamp1 - 2;
            int idsamp23 = idsamp2 - 2; int idsamp33 = idsamp3 - 2; samp = ZXP(in);
            del1 = cubicinterp(frac1, buf1[(idsamp1 + 1) & mask1], buf1[idsamp1 & mask1], buf1[(idsamp1 - 1) & mask1],
                               buf1[idsamp13 & mask1]);
            sum1 = cubicinterp(frac2, buf2[(idsamp2 + 1) & mask2], buf2[idsamp2 & mask2], buf2[(idsamp2 - 1) & mask2],
                               buf2[idsamp23 & mask2])
            ...
            sum2 = cubicinterp(frac3, buf3[(idsamp3 + 1) & mask3], buf3[idsamp3 & mask3], buf3[(idsamp3 - 1) & mask3],
                               buf3[idsamp33 & mask3])
                - gain3 * sum1;
            output = sum2 - gain1 * samp; if (idsamp13 < 0 || idsamp23 < 0 || idsamp33 < 0) {
                buf1[phase & mask1] = samp;
                buf2[phase & mask2] = samp;
                buf3[phase & mask3] = samp;
                ZXP(out) = 0.f;
...
#define DefineFilterDelay(name)                                                                                        \
    (*ft->fDefineUnit)(#name, sizeof(name), (UnitCtorFunc)&name##_Ctor, (UnitDtorFunc)&FilterDelay_Dtor, 0);
...
#include "SC_Wasm_RegisterPlugin.h"
SC_WASM_REGISTER_FOR_PLUGIN(BhobFilt, BhobFilt)
