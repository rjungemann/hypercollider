...
void GaussTrig_Ctor(GaussTrig* unit) {
    RGET int nextSamples = (int)(SAMPLERATE / sc_max(ZIN0(0), 0.001f));
...
void Dgauss_next(Dgauss* unit, int inNumSamples) {
    if (inNumSamples) {
        ...
        float value =
            ((sqrt(-2 * log(unit->mParent->mRGen->frand())) * sin(unit->mParent->mRGen->frand() * twopi)) + 1) * 0.5;
...
void TBrownRand_Ctor(TBrownRand* unit) {
    ...
    if (unit->mCalcRate == calc_FullRate) {
        SETCALC(TBrownRand_next_a);
    } else {
        SETCALC(TBrownRand_next_k);
    }
    unit->m_trig = ZIN0(3);
}
...
void TGaussRand_next_k(TGaussRand* unit, int inNumSamples) {
    ...
    if (trig > 0.f && unit->m_trig <= 0.f) {
        RGET float value = ((sqrt(-2 * log(frand(s1, s2, s3))) * sin(frand(s1, s2, s3) * twopi)) + 1) * 0.5;
        value = Mirroring(0.0, 1.0, value);
...
void TGaussRand_Ctor(TGaussRand* unit) {
    ...
    ZOUT0(0) = unit->m_value * range + lo;
    if (unit->mCalcRate == calc_FullRate) {
        SETCALC(TGaussRand_next_a);
    } else {
        SETCALC(TGaussRand_next_k);
    }
    unit->m_trig = ZIN0(2);
}
...
void TBetaRand_next_a(TBetaRand* unit, int inNumSamples) {
    ...
    LOOP(
        inNumSamples, float invProb1 = 1 / ZXP(prob1); float invProb2 = 1 / ZXP(prob2); next = ZXP(trig);
...
void TBetaRand_Ctor(TBetaRand* unit) {
    ...
    ZOUT0(0) = unit->m_value = (value / sum) * (hi - lo) + lo;
    RPUT if (unit->mCalcRate == calc_FullRate) { SETCALC(TBetaRand_next_a); }
    else {
        SETCALC(TBetaRand_next_k);
    }
    unit->m_trig = ZIN0(4);
}
...
void Gendy4_next_k(Gendy4* unit, int inNumSamples) {
    ...
    do {
        ...
        remain -= nsmps;
        phase -= nsmps;
        LOOP(nsmps, ZXP(out) = midpnt; slope += curve; midpnt += slope;);
    } while (remain);
...
SC_WASM_REGISTER_FOR_PLUGIN(BhobNoise, BhobNoise)
