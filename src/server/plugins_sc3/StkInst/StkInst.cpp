...
#if defined(SC_WASM) && defined(SC_WASM_STK_RAWWAVES)
static constexpr const char* kWasmStkRawwavePath = "/stksc/rawwaves/";
#endif

#ifdef SC_WASM
static bool stk_wasm_is_sample_backed(int number)
{
    switch (number) {
    case 10: // Mandolin
    case 11: // Rhodey
...
static bool stk_wasm_instrument_supported(int number)
{
    if (number < 0 || number >= NUM_INSTS)
        return false;
#ifdef SC_WASM_STK_RAWWAVES
    return true;
#else
    return !stk_wasm_is_sample_backed(number);
#endif
}
#endif
...
int voiceByNumber(int number, Instrmnt **instrument)
{
  int temp = number;
#ifdef SC_WASM
    if (!stk_wasm_instrument_supported(number)) {
        Print("StkInst: instrument %d requires rawwave assets and is not enabled in this WASM build\n", number);
        return -1;
    }
#endif
...
#if defined(SC_WASM) && defined(SC_WASM_STK_RAWWAVES)
    Stk::setRawwavePath(kWasmStkRawwavePath);
#endif
...
SC_WASM_REGISTER_FOR_PLUGIN(StkUnit, StkUnit)
