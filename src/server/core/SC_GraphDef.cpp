...
#ifdef SC_WASM
        scprintf("SC_WASM GraphDef_Define: name=%s\n", (char*)graphDef->mNodeDef.mName);
        count++;
#endif
...
#ifdef SC_WASM
    scprintf("SC_WASM GraphDef_Define: total=%d\n", count);
#endif
}
...
#ifdef SC_WASM
        scprintf("SC_WASM GraphDef_Recv: size=%d result=%s\n", (int)size, inList ? "ok" : "null");
#endif
