...
#include "../../common/sndfile_helpers.hpp"
...
// Detect SSE support via compiler macros (replaces boost/predef/hardware.h)
#if defined(__SSE__)
#    include <xmmintrin.h>
#endif
...
// Replaces BOOST_HW_SIMD_X86 (boost.predef) with the standard SSE
// preprocessor symbol that GCC/Clang/MSVC all define when SSE codegen
// is enabled.
#if defined(__SSE__)
...
World* World_New(WorldOptions* inOptions) {
    ...
    try {
        ...
        world->hw->mQuitProgram = new sc_quit_semaphore(0);
