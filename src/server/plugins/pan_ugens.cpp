...
#include "../../common/align_utils.hpp"
...
void LinPan2_Ctor(LinPan2* unit) {
    if (INRATE(1) == calc_FullRate) {
        ...
        if (nova::is_aligned(BUFLENGTH, 16))
...
void Balance2_Ctor(Balance2* unit) {
    if (INRATE(2) == calc_FullRate) {
        ...
        else if (nova::is_aligned(BUFLENGTH, 16))
...
void XFade2_next_ak_nova_64(XFade2* unit, int inNumSamples) {
    ...
    if (pos != unit->m_pos || unit->m_level != level) {
        ...
        nova::mix_vec_simd<64>(OUT(0), IN(0), slope_argument(leftamp, leftampslope), IN(1),
                               slope_argument(rightamp, rightampslope));
...
static inline void pan4_normalize_pos(float& xpos, float& ypos) {
...
void PanAz_next_aa(PanAz* unit, int inNumSamples) {
    ...
    for (int i = 0; i < numOutputs; ++i) {
        ...
        LOOP1(
            inNumSamples, float chanpos = ((ZXP(thePos) * alignedPosFac + alignedPosConst) - i) * rwidth;
            chanpos = chanpos - range * sc_floor(rrange * chanpos);
