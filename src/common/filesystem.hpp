...
public:
     ...
     * - one of "windows", "osx", or "linux", but that is not the name of the current platform.
     */
...
private:
    ...
    /** Of the three strings "windows", "osx", "linux", returns true if `p` is one of them
     * and `p` doesn't correspond to this platform. */
