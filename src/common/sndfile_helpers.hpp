...
// Case-insensitive C-string equality. Replaces boost::iequals (formerly
// pulled from <boost/algorithm/string/predicate.hpp>) — the only call
// pattern in this file is `iequals(c_str, "LITERAL")`, so a tiny inline
// strcasecmp wrapper is sufficient. Phase H Option B / additional
// cleanup, see docs/BOOST_ELIMINATION_PLAN.md.
#if defined(_WIN32)
#    include <cstring>
static inline bool sc_iequals(const char* a, const char* b) {
...
#if defined(SC_WASM_SNDFILE_COMPAT)

#    include "wasm_sndfile_compat.hpp"

#    include "string.h"

inline bool iequals(const char* a, const char* b) { return sc_iequals(a, b); }

static inline int headerFormatFromString(const char* name) {
    if (!name || iequals(name, "AIFF") || iequals(name, "AIFC") || iequals(name, "WAVE") || iequals(name, "WAV")
        || iequals(name, "RIFF"))
...
static inline int sampleFormatFromString(const char* name) {
    ...
    if (iequals(name, "int8"))
        return SF_FORMAT_PCM_S8;
    if (iequals(name, "int16"))
        return SF_FORMAT_PCM_16;
    if (iequals(name, "int24"))
        return SF_FORMAT_PCM_24;
    if (iequals(name, "int32"))
        return SF_FORMAT_PCM_32;
    if (iequals(name, "float"))
        return SF_FORMAT_FLOAT;
    if (iequals(name, "double"))
...
#else // not NO_LIBSNDFILE and not SC_WASM_SNDFILE_COMPAT
