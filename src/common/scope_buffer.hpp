...
class scope_buffer {
    ...
    */
    ...
    struct data_desc {
        // sh_float_ptr default-constructs to its null sentinel, no
        // explicit init needed (was `data(0)` under boost — that
        // overload is intentionally absent from nova::offset_ptr).
        data_desc(): frames(0), changed(false) {}
        sh_float_ptr data;
        unsigned int frames;
        atomic<bool> changed;
    } _state[3];
...
private:
    ...
    bool allocate(scope_buffer_pool& pool, unsigned int channels, unsigned int size) {
        ...
        // Pointer arithmetic happens on the raw pointer, then assigns
        // back to sh_float_ptr (which recomputes the offset relative
        // to its own address).
        float* raw = (float*)pool.allocate(asset_size * 3 * sizeof(float));
        if (raw == nullptr)
            return false;
        _data = raw;

        _state[0].data = raw;
        _state[1].data = raw + asset_size;
        _state[2].data = raw + asset_size + asset_size;
