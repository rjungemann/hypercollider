...
inline void* MALLOC ASSUME_ALIGNED(64) malloc_aligned(std::size_t bytes) {
    // C11 std::aligned_alloc requires `bytes` be a multiple of the
    // alignment. Emscripten's wasi-sdk libc enforces this strictly and
    // returns NULL otherwise (AllocPool's ctor then throws, which is an
    // `unreachable` trap under -fno-exceptions). Round up so callers
    // don't have to care.
    constexpr std::size_t mask = static_cast<std::size_t>(malloc_memory_alignment) - 1;
    bytes = (bytes + mask) & ~mask;
    return std::aligned_alloc(malloc_memory_alignment, bytes);
}

inline void free_aligned(void* ptr) { return std::free(ptr); }
