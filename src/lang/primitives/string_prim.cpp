...
class regex_lru_cache {
    int regex_flags;
    std::list<std::pair<std::string, std::regex>> re_list;  // MRU order (front = most recent)
    std::unordered_map<std::string, std::list<std::pair<std::string, std::regex>>::iterator> re_map;  // pattern -> list iterator

    void pop_lru() {
        auto& back_entry = re_list.back();
        re_map.erase(back_entry.first);
        re_list.pop_back();
    }

public:
    regex_lru_cache(int regex_flags = std::regex_constants::ECMAScript):
    ...
    std::regex const& get_regex(const char* str, size_t size) {
        std::string pattern_str(str, size);

        // Check if regex is in cache
        auto it = re_map.find(pattern_str);
        if (it != re_map.end()) {
            auto& list_it = it->second;
            // Move to front (most recently used)
            re_list.splice(re_list.begin(), re_list, list_it);
            return list_it->second;
        }

        // Not in cache - add new regex
        if (re_list.size() >= 64) {
            pop_lru();
        }

        re_list.emplace_front(pattern_str, std::regex(pattern_str, std::regex_constants::ECMAScript));
        re_map[pattern_str] = re_list.begin();
        return re_list.front().second;
    }
};
...
int prString_ReplaceRegex(struct VMGlobals* g, int numArgsPushed) {
    // caches the last 64 std::regex instances.
    static detail::regex_lru_cache regex_lru_cache(std::regex_constants::ECMAScript);
    ...
    try {
        ...
        std::regex_replace(std::back_inserter(out), source_start, source_start + source_size, pattern, replace);
...
int prString_Regexp(struct VMGlobals* g, int numArgsPushed) {
    ...
    try {
        ...
        // Original boost code passed `match_nosubs | match_any` here.
        // `match_nosubs` is a regex *construction* flag in std::regex,
        // not a search flag, so it doesn't belong here. `match_any`
        // ("return any successful match") has no std equivalent — std
        // already returns the first successful match, which is what
        // this primitive wants. Drop both → use default search flags.
        const char* stringStart = slotRawString(b)->s + start;
        const char* stringEnd = stringStart + stringlen;
        bool res = regex_search(stringStart, stringEnd, pattern);
