...
#include "hungarian.h" // header-only; namespace schungarian::solve<T>(matrix)
...
static int prArrayMultiChanExpand(struct VMGlobals* g, int numArgsPushed) {
    ...
    for (j = 0; j < size; ++j) {
        ...
        } else if (IsObj(slot) && isKindOf(slotRawObject(slot), class_sequenceable_collection)
                   && !isKindOfSlot(slot, class_string)) {
            // Preserve original behavior: only Arrays are supported by this primitive.
            return errFailed;
        }
    }
    ...
    for (i = 0; i < maxlen; ++i) {
        ...
        for (j = 0; j < size; ++j) {
            slot = slots1 + j;
            if (isKindOfSlot(slot, class_array) && slotRawObject(slot)->size > 0) {
                PyrSlot* slotToCopy;
...
static int arrayAtIdentityHash(PyrObject* array, PyrSlot* key) {
    PyrSlot *slots, *test;
...
static int prArray_AtIdentityHash(struct VMGlobals* g, int numArgsPushed) {
    PyrSlot *a, *b;
    PyrObject* array;
    int index;
...
static int arrayAtIdentityHashInPairs(PyrObject* array, PyrSlot* key) {
    PyrSlot *slots, *test;
    unsigned int i, start, end, hash, maxHash;
...
static int prArray_AtIdentityHashInPairs(struct VMGlobals* g, int numArgsPushed) {
    PyrSlot *a, *b;
    PyrObject* array;
    int index;
