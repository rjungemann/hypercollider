...
#include "symbol.h"
#include "file_prim.h"
#include "read_write_macros.h"
#include "sc_base.h"
#include "sc_popen.h"

/* SuperCollider newer headers*/
#include "sndfile_helpers.hpp"
#include "filesystem.hpp" // resolveIfAlias
#include "codecvt.hpp" // utf8_str_to_path, path_to_utf8_str
...
int prFileMTime(struct VMGlobals* g, int numArgsPushed) {
    ...
    const fs::path& p = SC_Codecvt::utf8_str_to_path(filename);
    std::error_code _ec;
    auto mtime = fs::last_write_time(p, _ec);
    if (_ec) { SetInt(a, 0); return errNone; }
...
int prFileType(struct VMGlobals* g, int numArgsPushed) {
    ...
    const fs::path& p = SC_Codecvt::utf8_str_to_path(filename);
    std::error_code _ec;
    fs::file_status s(fs::symlink_status(p, _ec));
...
int prFileSize(struct VMGlobals* g, int numArgsPushed) {
    ...
    const fs::path& p = SC_Codecvt::utf8_str_to_path(filename);
    std::error_code _ec;
    uintmax_t sz = fs::file_size(p, _ec);
    if (_ec) sz = 0;
    SetInt(a, sz);
    return errNone;
}
...
#else // !NO_LIBSNDFILE and !SC_WASM_SNDFILE_COMPAT
...
#endif // !NO_LIBSNDFILE || SC_WASM_SNDFILE_COMPAT
