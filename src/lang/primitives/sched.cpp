...
// Same WASI-compatible mini-semaphore as SC_HiddenWorld.h. The desktop
// build keeps using std::counting_semaphore; under SC_WASM the libc++
// constructor traps because there's no pthread infrastructure to back
// it. Single producer / single consumer here too — atomic suffices.
#if defined(SC_WASM)
namespace detail_sched_sem {
...
static detail_sched_sem::mini_semaphore gResyncThreadSemaphore(0);
#else
static std::counting_semaphore<std::numeric_limits<int>::max()> gResyncThreadSemaphore(0);
#endif
...
SCLANG_DLLEXPORT_C void schedCleanup() {
    gResyncThreadSemaphore.release();
    gResyncThread.join();
}
...
void resyncThread() {
    while (true) {
        if (gResyncThreadSemaphore.try_acquire_for(std::chrono::seconds(20)))
            return;
