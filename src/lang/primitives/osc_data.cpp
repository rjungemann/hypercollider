...
EM_JS(int, sc_wasm_forward_osc_js, (const char* msg, int size), {
    const bridge = (typeof Module.__sc_wasm_forward_osc_to_engine === 'function')
        ? Module.__sc_wasm_forward_osc_to_engine
        : ((typeof globalThis.__sc_wasm_forward_osc_to_engine === 'function')
            ? globalThis.__sc_wasm_forward_osc_to_engine
            : null);

    if (!bridge) {
        return -10; // bridge function not installed
    }

    try {
        const bytes = new Uint8Array(Module.HEAPU8.buffer, msg, size);
        return bridge(bytes);
    } catch (e) {
        return -11; // bridge execution exception
    }
});
#endif
...
#ifdef SC_WASM
static int gWasmLangPort = 57120;
#endif
...
static int netAddrSend(PyrObject* netAddrObj, int msglen, char* bufptr, bool sendMsgLen = true) {
    ...
    static int s_wasmSendTraceCount = 0;
    const bool trace = s_wasmSendTraceCount < 24;
    if (trace) {
        if (msglen > 0 && bufptr && bufptr[0] == '/') {
            postfl("WASM OSC send msg='%s' len=%d", bufptr, msglen);
        } else if (msglen >= 8 && bufptr && memcmp(bufptr, "#bundle", 7) == 0) {
            postfl("WASM OSC send bundle len=%d", msglen);
        } else {
            postfl("WASM OSC send packet len=%d", msglen);
        }
    }

    // Split-module Browser IDE runtime: forward OSC bytes via JS bridge to the
    // active scsynth module instance owning the rendered world.
    const int forwardResult = sc_wasm_forward_osc_js(bufptr, msglen);
    if (forwardResult != 0) {
        postfl("WASM: netAddrSend forward bridge failed (code=%d)", forwardResult);
        return errFailed;
    }
    if (trace) {
        postfl("WASM OSC send forwarded ok");
        s_wasmSendTraceCount++;
    }
    return errNone;
#endif
    ...
    if (IsPtr(netAddrObj->slots + ivxNetAddr_Socket)) {
        ...
        if (addr == 0) {
#ifdef SC_WASM
            // Route local server messages through the in-process OSC bridge.
            const int dispatchResult = hc_wasm_osc_dispatch(0, bufptr, msglen);
            if (dispatchResult != 0) {
                postfl("WASM: OSC dispatch failed (no registered world)");
            }
#else
...
#ifdef SC_WASM
        // Browser builds do not support raw UDP sockets, so route OSC sends
        // through the in-process bridge.
    const int dispatchResult = hc_wasm_osc_dispatch(0, bufptr, msglen);
        if (dispatchResult != 0) {
            postfl("WASM: In-process OSC dispatch unavailable (no registered world)");
        }
#else
        using namespace asio;
...
static int prNetAddr_SetBroadcastFlag(VMGlobals* g, int numArgsPushed) {
    ...
    asio::error_code ec;
    asio::socket_base::broadcast option(IsTrue(g->sp));
...
static PyrObject* ConvertReplyAddress(ReplyAddress* inReply) {
    ...
    PyrSlot* slots = obj->slots;
#ifdef SC_WASM
    SetInt(slots + 0, static_cast<int>(inReply->mAddress));
#else
...
#ifdef SC_WASM
    // WASM runs OSC transport in-process through SC_Wasm_OscShim.
    gWasmLangPort = port;
    gUDPport = nullptr;
#else
#ifdef _WIN32
...
void closeAllCustomPorts() {
#ifdef SC_WASM
    // Browser runtime does not create custom UDP/TCP ports.
...
#ifdef SC_WASM
    // Explicit WASM network model: only local loopback and dotted IPv4 parsing.
...
static int prLocalIP(VMGlobals* g, int numArgsPushed) {
    ...
    if (NotNil(a)) {
        ...
        if (slotStrVal(a, addr, 64) != errNone)
            return errWrongType;
        // Keep deterministic loopback answer for browser runtime.
        if (strcmp(addr, "127.0.0.1") != 0 && strcmp(addr, "localhost") != 0)
            return errFailed;
    }

    PyrString* result = newPyrString(g->gc, "127.0.0.1", 0, true);
    if (!result)
        return errFailed;
...
#ifdef SC_WASM
        // Register the world with the OSC shim for in-process communication
        // Use world_id 1 for the default internal server
        sc_wasm_osc_register_world(1, gInternalSynthServer.mWorld);
        
        // Set up callback handler for messages from scsynth back to sclang
        // The callback signature is: void (*)(int world_id, const char* msg_data, size_t msg_size, ReplyAddress* reply)
        // We need to forward to the sclang OSC message handler
        // For now, use a simple wrapper that creates a PyrObject reply and calls PerformOSCMessage
        sc_wasm_osc_register_handler(1, [](int world_id, const char* msg_data, size_t msg_size, ReplyAddress* reply) {
            // For WASM, we receive messages from scsynth via callback
            // Convert to sclang's internal message format
            // Note: This is a simplified version - full implementation needs proper
            // reply object handling
            if (compiledOK && gMainVMGlobals) {
                PyrObject* replyObj = nullptr; // TODO: Convert ReplyAddress to PyrObject
                PerformOSCMessage((int)msg_size, msg_data, replyObj, 0, 0.0);
            }
        });
#endif
    }
...
int prQuitInProcessServer(VMGlobals* g, int numArgsPushed) {
    ...
    if (gInternalSynthServer.mWorld) {
#ifdef SC_WASM
        sc_wasm_osc_unregister_world(1);
#endif
