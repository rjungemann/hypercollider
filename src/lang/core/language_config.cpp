...
#include "filesystem.hpp" // getDirectory

#include "sc_base.h" // postfl
...
bool SC_LanguageConfig::writeLibraryConfigYAML(const Path& fileName) {
    { std::error_code _ec; if (!fs::exists(fileName.parent_path(), _ec) || _ec) return false; }
...
bool SC_LanguageConfig::readLibraryConfig(bool standalone) {
    ...
    { std::error_code _ec; if (fs::exists(gConfigFile, _ec) && !_ec)
    ...
    if (!configured && !standalone) {
        ...
        { std::error_code _ec; if (fs::exists(userYamlConfigFile, _ec) && !_ec)
        ...
        if (!configured) {
            ...
            { std::error_code _ec; if (fs::exists(globalYamlConfigFile, _ec) && !_ec)
