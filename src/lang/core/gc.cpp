...
#ifdef SC_WASM
#include "wasm_stubs.h"  // For setPostFile stub
#endif
...
PyrObject* PyrGC::NewPermanent(size_t inNumBytes, std::int64_t inFlags, std::int64_t inFormat) {
    // obtain size info
    int32 alignedSize = (inNumBytes + kAlignMask) & ~kAlignMask; // 64 byte align
...
HOT PyrObject* PyrGC::New(size_t inNumBytes, std::int64_t inFlags, std::int64_t inFormat, bool inRunCollection) {
    ...
    // obtain size info
    const size_t alignedSize = (inNumBytes + kAlignMask) & ~kAlignMask; // 64 byte align
...
HOT void PyrGC::DoPartialScan(int32 inObjSize) {
    ...
    mNumToScan = sc_max(mNumToScan, 0);
}
