...
#include "primitive.h"
#include "object_proto.h"
#include "primitive_proto.h"
#include "kernel_proto.h"
#include "init_alloc.h"
#include "predefined_symbols.h"
#ifdef SC_WASM
// For WASI builds, use clock_gettime; otherwise use emscripten_get_now
#if !defined(SC_WASM_WASI)
#    include <emscripten.h>
#endif
#include "wasm_platform.h"
// Task 8: content cache provided by HC_Wasm_Eval.cpp via wasm_classlib_preload().
// Returns heap-allocated (malloc) content for a MEMFS path, or nullptr on miss.
extern "C" const char* wasm_classlib_cache_lookup(const char* memfs_path);
// Sub-phase timing for compileLibrary, readable from SC_Wasm_Eval.cpp after boot.
struct SCWasmBootTiming {
    double shutdown_ms   = 0;
    double passone_ms    = 0;
    double deptree_ms    = 0;
    double succeeded_ms  = 0;
    double initruntime_ms = 0; // Task 10a: C++ initRuntime() only
    double startup_ms    = 0; // Task 10a: SC runInterpreter(s_startup) only
};
static SCWasmBootTiming s_wasm_boot_timing;
extern "C" void sc_wasm_get_boot_timing(double* shutdown, double* passone,
                                         double* deptree, double* succeeded,
                                         double* initruntime, double* startup) {
    *shutdown    = s_wasm_boot_timing.shutdown_ms;
    *passone     = s_wasm_boot_timing.passone_ms;
    *deptree     = s_wasm_boot_timing.deptree_ms;
    *succeeded   = s_wasm_boot_timing.succeeded_ms;
    *initruntime = s_wasm_boot_timing.initruntime_ms;
    *startup     = s_wasm_boot_timing.startup_ms;
}
#endif
...
#include "filesystem.hpp" // getDirectory, resolveIfAlias, isStandalone
#include "codecvt.hpp" // path_to_utf8_str
...
bool startLexer(PyrSymbol* fileSym, const fs::path& p, int startPos, int endPos, int lineOffset) {
    ...
    if (!fileSym->u.source) {
        try {
            ...
            std::error_code _sz_ec;
            size_t sz = static_cast<size_t>(fs::file_size(p, _sz_ec));
            if (_sz_ec) {
                error("Could not get file size of %s: %s.\n",
                      SC_Codecvt::path_to_utf8_str(p).c_str(),
                      _sz_ec.message().c_str());
                return false;
            }
...
#ifdef SC_WASM
        // Task 8: serve content from the pre-loaded cache (built from
        // hclang_classlib.pack) so startLexer skips the MEMFS fopen/fstat/fread.
        if (const char* cached = wasm_classlib_cache_lookup(path_c_str))
            fileSym->u.source = const_cast<char*>(cached);
#endif
...
#ifdef SC_WASM
        double _tir0 = sc_wasm_get_now() * 1000.0;  // convert seconds to ms
#endif
        compiledOK = initRuntime(gMainVMGlobals, 128 * 1024, pyr_pool_runtime);
#ifdef SC_WASM
        s_wasm_boot_timing.initruntime_ms = (sc_wasm_get_now() * 1000.0) - _tir0;
#endif
...
#ifdef SC_WASM
            double _tst0 = sc_wasm_get_now() * 1000.0;
#endif
            runInterpreter(g, s_startup, 1);
#ifdef SC_WASM
            s_wasm_boot_timing.startup_ms = (sc_wasm_get_now() * 1000.0) - _tst0;
#endif
            g->canCallOS = false;
...
#ifdef SC_WASM
    double _t0 = sc_wasm_get_now() * 1000.0;
    shutdownLibrary();
    s_wasm_boot_timing.shutdown_ms = (sc_wasm_get_now() * 1000.0) - _t0;
#else
    shutdownLibrary();
#endif
...
#ifdef SC_WASM
    double _t1 = sc_wasm_get_now() * 1000.0;
    bool res = passOne();
    s_wasm_boot_timing.passone_ms = (sc_wasm_get_now() * 1000.0) - _t1;
#else
    bool res = passOne();
#endif
    if (res) {
        if (!compileErrors) {
#ifdef SC_WASM
            double _t2 = sc_wasm_get_now() * 1000.0;
#endif
            buildDepTree();
            traverseFullDepTree();
            traverseFullDepTree2();
#ifdef SC_WASM
            s_wasm_boot_timing.deptree_ms = (sc_wasm_get_now() * 1000.0) - _t2;
#endif
            flushPostBuf();
        ...
        flushPostBuf();
#ifdef SC_WASM
        double _t3 = sc_wasm_get_now() * 1000.0;
        compileSucceeded();
        s_wasm_boot_timing.succeeded_ms = (sc_wasm_get_now() * 1000.0) - _t3;
#else
        compileSucceeded();
#endif
    } else {
