...
#ifdef SC_WASM
#include "wasm_stubs.h"  // For WASM stubs (error, initGUI)
#endif
...
#  ifdef SC_WASM_WASI
// SIGINT polling: every 4096 dispatches, ask the host whether a Ctrl-C
// has come in. The host's hc_host_poll_interrupt clears its own flag, so
// each Ctrl-C is consumed exactly once. 4096 bytecodes is ~40 µs at
// interpreter speeds, ~4 µs in AOT — fast enough to feel responsive,
// rare enough that the per-bytecode overhead is unmeasurable.
extern "C" {
__attribute__((import_module("env"), import_name("hc_host_poll_interrupt")))
int32_t hc_host_poll_interrupt(void);
}
static uint32_t s_interrupt_check_counter = 0;
#    define dispatch_opcode                                                                                            \
        if ((++s_interrupt_check_counter & 0xfff) == 0 && hc_host_poll_interrupt()) { \
            g->wasiEscape = 1;                                                                                          \
        }                                                                                                              \
        if (g->wasiEscape) { g->wasiEscape = 0; return; }                                                              \
        op1 = ip[1];                                                                                                   \
        ++ip;                                                                                                          \
