...
#if defined(_MSC_VER) || defined(SC_WASM)
...
static ColumnDescriptor* prepareColumnTable(ColumnDescriptor* sels, int numSelectors) {
    ...
    for (int i : std::views::iota(0, symbolTable->TableSize())) {
        PyrSymbol* sym = symbolTable->Get(i);
        if (sym && (sym->flags & sym_Selector))
...
void buildBigMethodMatrix() {
    ...
    (void)helperThreadCount; // used implicitly via std::async thread count
    ...
    MEMFAIL(sels);
    ColumnDescriptor* filledSelectors = nullptr;  // Declare outside conditional for proper scope
#if defined(SC_WASM)
    filledSelectors = prepareColumnTable(sels, numSelectors);
#else
...
#if defined(SC_WASM)
    PyrClass** filledClassIndices = fillClassIndices(classes);
#else
...
#if !defined(SC_WASM)
    filledSelectors = filledSelectorsFuture.get();
#endif
    // Note: previously used `std::views::iota(...) | std::views::stride(N)`
    // which is C++23. Rewritten as an explicit stride loop so we stay
    // on C++20 (set in CMakeLists.txt:94 + the WASI toolchain).
    size_t selectorsPerJob = numSelectors / cpuCount / 2;
#if defined(SC_WASM)
    // WASM: synchronous, single-threaded.
    for (size_t beginSelectorIndex = selectorsPerJob;
         beginSelectorIndex < numSelectors;
         beginSelectorIndex += selectorsPerJob) {
        size_t endSelectorIndex = std::min(beginSelectorIndex + selectorsPerJob, numSelectors);
        calcRowStats(bigTable, filledSelectors, numClasses, numSelectors, beginSelectorIndex, endSelectorIndex);
    }
    ...
    for (size_t beginSelectorIndex = selectorsPerJob;
         ...
         beginSelectorIndex += selectorsPerJob) {
        size_t endSelectorIndex = std::min(beginSelectorIndex + selectorsPerJob, numSelectors);
        auto future = std::async(std::launch::async, calcRowStats, bigTable, filledSelectors, numClasses, numSelectors,
    ...
    // bin sort the class rows to the new ordering
    // post("reorder rows\n");
    // Same C++23 stride rewrite as above.
    size_t classesPerJob = numClasses / cpuCount / 2;
#if defined(SC_WASM)
    for (size_t beginClassIndex = classesPerJob;
         beginClassIndex < numClasses;
         beginClassIndex += classesPerJob) {
        size_t endClassIndex = std::min(beginClassIndex + classesPerJob, numClasses);
        binsortClassRows((PyrMethod const**)bigTable, sels, numSelectors, beginClassIndex, endClassIndex);
    }
    ...
    for (size_t beginClassIndex = classesPerJob;
         ...
         beginClassIndex += classesPerJob) {
        size_t endClassIndex = std::min(beginClassIndex + classesPerJob, numClasses);
        auto future = std::async(std::launch::async, binsortClassRows, (PyrMethod const**)bigTable, sels,
...
#endif
{
    ...
    if (IsObj(&classobj->subclasses)) {
        ...
        if (numSubclasses) {
#if defined(_MSC_VER)
            if (numSubclasses <= 2) {
                for (int subClassIndex : std::views::iota(0, numSubclasses))
                ...
                for (int subClassIndex : std::views::iota(1, numSubclasses)) {
            ...
            // For WASM, execute all subclasses synchronously
            for (int subClassIndex : std::views::iota(0, numSubclasses))
                result += fillClassRow(slotRawClass(&subclasses->slots[subClassIndex]), bigTable);
#else
            if (numSubclasses <= 2) {
                ...
                VectorOfFutures subclassResults;
                for (int subClassIndex : std::views::iota(1, numSubclasses)) {
                    auto subclassResult = std::async(std::launch::async, fillClassRow,
                                                     slotRawClass(&subclasses->slots[subClassIndex]), bigTable);
                    subclassResults.emplace_back(std::move(subclassResult));
                }
