...
class MacroExpander {
  constructor() {
    ...
    // Note: _loadStdlib is async, call it separately if needed
    // For browser use, we'll load stdlib on first use
    this._stdlibLoaded = false;
  }

  async init() {
    if (!this._stdlibLoaded) {
      await this._loadStdlib();
      this._stdlibLoaded = true;
    }
    return this;
  }
  ...
  async _loadStdlib() {
    const { Lexer }  = await import('./lhc_lexer.js');
    const { Parser } = await import('./lhc_parser.js');
